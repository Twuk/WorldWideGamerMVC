    ...
    internal sealed class Configuration : DbMigrationsConfiguration<WorldWideGamerMVC.Models.ApplicationDbContext>
    {
        ...
        protected override void Seed(WorldWideGamerMVC.Models.ApplicationDbContext context)
        {
            ...
            manager.Create(user2, "Demiete6");
                ...
                var spel = new Game() { Naam = "Age of Empires 2", Regels = "Regels voor Age of Empires 2", TeamSpel = true };
                context.Games.Add(spel);
                context.SaveChanges();
                var spel2 = new Game() { Naam = "CounterStrike", Regels = "Regels voor CounterStrike", TeamSpel = true };
                context.Games.Add(spel2);
                context.SaveChanges();

                var spel3 = new Game() { Naam = "HeartStone", Regels = "Regels voor HeartStone", TeamSpel = false };
                context.Games.Add(spel3);
                context.SaveChanges();

                var spel4 = new Game() { Naam = "League of Legends", Regels = "Regels voor League of Legends", TeamSpel = true };
                context.Games.Add(spel4);
                context.SaveChanges();

                var spel5 = new Game() { Naam = "Fifa", Regels = "Regels voor Fifa", TeamSpel = false };
